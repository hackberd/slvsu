...
//					((Uebersicht) e.getSource()).setToolTipText(text);
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (uebersichtPanel.getBalken() != null && uebersichtPanel.getStartSelection().getX() == uebersichtPanel.getEndSelection().getX() && uebersichtPanel.getStartSelection().getY() == uebersichtPanel.getEndSelection().getY()) {
			Boolean hit = false;
			for (Balken bar : uebersichtPanel.getBalken()) {
			...
			if (!hit) { // Deselecten
				...
				this.uebersichtPanel.data.getData().setCurrentZeitscheibe(null);
				this.uebersichtPanel.data.getData().controller.receive();
			}
		}
